...
export default function AdminLayout() {
  ...
  const navItems = [
    ...
    { path: '/admin/usuarios', label: 'Equipe', icon: Users }
  ];

  const currentPageTitle = navItems.find(item => item.path === location.pathname)?.label || 'Sistema';

  return (
    <div className="min-h-screen bg-[var(--bg-app)] text-[var(--text-main)] flex overflow-hidden transition-colors duration-500">
      
      {/* Background Decorative Elements */}
      <div className="fixed top-[-10%] left-[-5%] w-[40%] h-[40%] rounded-full bg-blue-600/5 blur-[120px] pointer-events-none" />
      ...
      >
        <div className="h-24 flex items-center px-8 border-b border-[var(--border-main)]/50">
          <div className="flex items-center gap-4 group cursor-pointer">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-bold text-blue-500 tracking-[0.2em] uppercase mt-1">Admin Panel</span>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-8">
          <div>
            <p className="px-4 text-[10px] font-black text-[var(--text-muted)] uppercase tracking-[0.2em] mb-4 opacity-50">Global Control</p>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 mt-1 opacity-60">
                ...
                <span className="text-[9px] font-black uppercase tracking-[0.2em]">Network Node: AWS-SA-1</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-4 pl-4 border-l border-[var(--border-main)] hidden sm:flex">
              <div className="flex flex-col items-end">
                <span className="text-sm font-black text-[var(--text-main)] tracking-tight uppercase leading-none">{user?.nome}</span>
                <span className="text-[10px] text-blue-500 font-bold uppercase tracking-widest mt-1">Super Admin</span>
              </div>
              <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-gray-800 to-gray-700 border border-[var(--border-main)] flex items-center justify-center shadow-lg group-hover:scale-105 transition-transform cursor-pointer">
                <span className="text-lg font-black text-white">{user?.nome?.charAt(0)}</span>
              </div>
            </div>
          </div>
        </header>
