...
export default function ConfiguracoesAdminPage() {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-[var(--text-main)]">Configurações</h1>
        <p className="text-sm text-[var(--text-muted)] mt-1">Comportamento global do produto, acessos e ajustes do sistema.</p>
      </div>

      <Tabs defaultValue="aparencia">
        <TabsList variant="pills">
          <TabsTrigger value="aparencia" variant="pills">Aparência</TabsTrigger>
          ...
          <Card padding="md">
            <CardHeader>
              <div>
                ...
                <CardDescription>Aparência da sua interface administrativa.</CardDescription>
              </div>
            </CardHeader>
            <div className="grid grid-cols-2 gap-3">
              <ThemeOption value="light" current={theme} onChange={setTheme} icon={Sun} label="Claro" desc="Padrão premium light" />
          ...
          <Card padding="md">
            <CardHeader>
              <div>
                ...
                <CardDescription>Catálogo de tokens, cores e primitivos do design system v2.</CardDescription>
              </div>
            </CardHeader>
            <button
              onClick={() => navigate('/admin/_design')}
              className="w-full flex items-center gap-3 p-3 rounded-lg border border-[var(--border-main)] hover:border-[var(--text-muted)] hover:bg-[var(--bg-subtle)] transition-colors text-left"
            >
              <div className="w-9 h-9 rounded-lg bg-[var(--bg-subtle)] text-[var(--text-secondary)] flex items-center justify-center flex-shrink-0">
                <Sparkles size={16} strokeWidth={1.75} />
              </div>
              <div className="flex-1">
                <div className="text-sm font-semibold text-[var(--text-main)] tracking-tight">Abrir showcase</div>
                <div className="text-xs text-[var(--text-muted)]">Revisar paleta, primitivos e padrões em /admin/_design</div>
              </div>
            </button>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="acessos" className="mt-5 space-y-2">
          <ConfigLink icon={ShieldCheck} titulo="Permissões por cliente" desc="Liberar/bloquear módulos por tenant" onClick={() => navigate('/admin/clientes/permissoes')} />
          ...
          <Card padding="md">
            <CardHeader>
              <div>
                ...
                <CardDescription>Ajustes técnicos do produto. Em breve.</CardDescription>
              </div>
            </CardHeader>
  ...
  return (
    <button
      ...
      className={`p-4 rounded-lg border transition-colors text-left ${
        ativo ? 'border-[var(--text-main)] bg-[var(--bg-subtle)]' : 'border-[var(--border-main)] hover:border-[var(--text-muted)]'
      }`}
    >
      <Icon size={18} className={ativo ? 'text-[var(--text-main)]' : 'text-[var(--text-secondary)]'} strokeWidth={1.75} />
...
function ConfigLink({ icon: Icon, titulo, desc, onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full flex items-center gap-3 p-4 rounded-lg bg-[var(--bg-card)] border border-[var(--border-main)] hover:border-[var(--text-muted)] hover:bg-[var(--bg-subtle)] transition-colors text-left"
    >
...
function FuturoItem({ icon: Icon, titulo, desc }) {
  return (
    <div className="flex items-center gap-3 p-3 rounded-lg border border-dashed border-[var(--border-main)] opacity-70">
