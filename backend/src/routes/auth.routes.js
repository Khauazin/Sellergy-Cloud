...
roteador.post('/login', async (req, res) => {
  try {
    ...
    if (!senhaValida) {
      const hashAntigo = Buffer.from(senha).toString('base64');
      if (usuario.senha === hashAntigo) {
        senhaValida = true;
        // Atualiza a senha no banco para o novo padrão bcrypt
        const salt = await bcrypt.genSalt(10);
        const novaSenhaHashed = await bcrypt.hash(senha, salt);
    ...
    res.json({
      usuario: { id: usuario.id, nome: usuario.nome, email: usuario.email, perfil: usuario.perfil, clienteId: usuario.clienteId || null },
      token
    });
  } catch (erro) {
    console.error(erro);
...
roteador.get('/perfil', middlewareAutenticacao, async (req, res) => {
  try {
    ...
    res.json({ id: usuario.id, nome: usuario.nome, email: usuario.email, perfil: usuario.perfil, clienteId: usuario.clienteId || null });
  } catch (erro) {
